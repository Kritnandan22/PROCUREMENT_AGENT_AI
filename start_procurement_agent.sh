...
if ! "${python_bin}" -c "import oracledb, dotenv, openpyxl" >/dev/null 2>&1; then
  echo "Installing required Python packages..." >&2
  "${python_bin}" -m pip install -r requirements.txt
fi

"${python_bin}" install_claude_desktop.py
...
cmd=(
  "${python_bin}" tutorial_agentic_procurement_agent.py
  --engine "${engine}"
  --workflow "${workflow}"
...
if [[ -n "${organization_id}" ]]; then
  cmd+=(--organization-id "${organization_id}")
fi
